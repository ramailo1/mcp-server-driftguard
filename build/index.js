...
import { StateTransitionError, IntentMissingError } from './types.js';
// Create MCP Server
...
// ==================== TOOL: dg_init ====================
server.tool('dg_init', 'Initialize DriftGuard in the project. Creates .driftguard/ directory and recommends a strictness level based on project analysis.', {
...
server.tool('dg_propose_task', 'Propose a new task with defined scope and checklist. Transitions from IDLE to PLANNING state.', {
    ...
    try {
        ...
        const task = await stateManager.proposeTask(title, goal, allowedScopes, checklist, strictness);
...
// ==================== TOOL: dg_begin_step ====================
server.tool('dg_begin_step', 'Begin executing a step. Transitions from PLANNING or IDLE to EXECUTING state.', {
...
// ==================== TOOL: dg_checkpoint ====================
server.tool('dg_checkpoint', 'Save progress and atomically checkpoint completed work. Transitions from EXECUTING to IDLE state.', {
...
// ==================== TOOL: dg_panic ====================
server.tool('dg_panic', 'Emergency stop. Forces PANIC state and locks all write operations. Returns a help packet for debugging.', {
...
// ==================== TOOL: dg_status ====================
server.tool('dg_status', 'Get the current DriftGuard session status without changing state.', {}, async () => {
    try {
...
// ==================== TOOL: dg_verify ====================
server.tool('dg_verify', 'Run the test command to verify changes. Uses testCommand from the task contract.', {}, async () => {
    try {
...
server.tool('dg_explain_change', 'Generate a 3-bullet summary comparing intent vs actual changes.', {}, async () => {
    try {
...
server.tool('dg_set_test_command', 'Set or update the test command for the active task.', {
